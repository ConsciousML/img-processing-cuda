...
std::valarray<float> gauss_weight(Mat image, int y1, int x1, int y2, int x2, int conv_size, double stddev)
{
    auto b1 = conv(image, y1, x1, conv_size);
    auto b2 = conv(image, y2, x2, conv_size);
    std::valarray<float> res = {0, 0, 0};
    res = std::exp(-(std::pow(std::abs(b1 - b2), 2) / std::pow(stddev * 2.5, 2)));
    return res;
}

 std::valarray<float> cp(Mat image, int y, int x, int conv_size, double stddev)
{
    std::valarray<float> sum = {0, 0, 0};
    for (int j = 0; j < image.cols; j++)
        for (int i = 0; i < image.rows; i++)
        {
            auto tmp = gauss_weight(image, y, x, j, i, conv_size, stddev);
            sum += tmp;
        }
    return sum;
}

std::valarray<float> gauss_product(Mat image, Mat nlm_img, int y, int x, int conv_size, double stddev)
{
    std::valarray<float> sum = {0, 0, 0};
    ...
    for (int j = 0; j < nlm_img.cols; j++)
        ...
        {
            // f(p, q)
            auto tmp = gauss_weight(nlm_img, y, x, j, i, conv_size, stddev);
...
Mat non_local_means_cpu(Mat image, int conv_size)
{
    ...
    for (int j = 0; j < nlm_img.cols; j++)
        ...
        {
            //C(p)
            auto cp_var = cp(image, j, i, conv_size, stddev_val);

            //sum(v(q) * f(p,q)
            auto tmp = gauss_product(image, nlm_img, j, i, conv_size, stddev_val);
